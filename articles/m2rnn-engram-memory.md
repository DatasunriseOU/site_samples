...
The Megatron bridge around that layer is the glue for training inside a Megatron stack. The production bridge adapts the M2RNN layer to Megatron's mixer protocol: it accepts the standard module-construction arguments, transposes from `[seq, batch, hidden]` to `[batch, seq, hidden]` for the forward path, returns `(output, None)` so the surrounding residual path stays correct, and can run the pure-PyTorch recurrence rather than the kernel when compile compatibility matters. A small config bridge reconciles the prototype attribute names with Megatron's transformer configuration when both exist.
...
The config bridge between the prototype configuration and the Megatron transformer configuration is simplified in the production codebase: there is a single entry point, and M2RNN reads its fields directly from the Megatron config via stable attribute access. No extra shim layer.

### Engram in MegaCpp

the public Engram config sample defines `EngramConfig` and `NgramHashConfig`, both fail-closed. `EngramConfig.from_args` validates layer indices, n-gram orders, bottleneck dim, dropout, conv kernel, and conv impl (must be `"xla_safe"` or `"maxtext_depthwise"`). The `EngramBranch` implementation itself is what we want to carry over in shape — the module surface is small — with the doc-id threading, RMSNorm fix, and conv-impl selection all intact. `NgramHashConfig` is the adjacent feature (hashed n-gram token embeddings) and ships alongside but is a different module.

### Concepts in MegaCpp

The concept bank (`CBlock`) stays in the prototype stack for now. It is a clean module, it does what it says, but we have not promoted it into the MegaCpp feature set. The reason is pragmatic: in ablation runs the concept bank was additive but small, and it is parameter-heavy at useful `n_concepts`. We kept the code and the design doc reference ([Memorizing Transformers — Wu et al., arXiv 2203.08913]; [Flamingo cross-attention — Alayrac et al., 2022]) so a future preset can turn it back on without re-deriving the module. For MegaCpp v1 the memory subsystem is M2RNN + Engram + Mamba + attention, not five tiers.

## Ablations and what we kept

The changelog tells a consistent story about what survives contact with real hardware:
...
- Concept bank (`CBlock`) is off by default in the MegaCpp production codebase. Turning it on is an ablation, not a shippable configuration in v1.
- Keep M2RNN outside whole-model compile. `regional_compile` only, with the step loop under `@torch.compiler.disable`.

## References

- the research M2RNN module — matrix-state RNN layer, XMA/Triton/torch forward paths
- the Megatron bridge for M2RNN — Megatron mixer-protocol wrapper for M2RNN
- the Engram module — causal n-gram branch with optional gating and depthwise conv
- the concept-retrieval module — `CBlock`, the cross-attention to a learned concept bank
- the public recurrent spec sample — production M2RNN mixer with Triton kernel hookup
- the public M2RNN Triton sample — fused Triton scan
- the MegaCpp production-codebase M2RNN and Engram config surface — fail-closed config surfaces
- [Accelerated model architectures / open-lm-engine — XMA reference]
- [Memorizing Transformers — Wu et al., arXiv 2203.08913]
- [Flamingo — Alayrac et al., 2022]
- [DeepSeek-style gating — arXiv 2601.07372]
