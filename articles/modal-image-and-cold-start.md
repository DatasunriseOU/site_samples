...
We use Modal for work shapes where the startup-to-useful-work ratio matters: a 10-step FA4 smoke, a 100-step preset sanity check, a throughput regression after a kernel change. If container startup costs 20 minutes on a 10-minute job, Modal is a worse version of reserved H200 systems. If startup is under 90 seconds, Modal is a strictly better tool for that shape of work. The entire image-and-volume design in MegaCpp is organized around pushing the expected startup below that threshold for the jobs we care about, while accepting a worse tail for the first run after any stack bump.
...
The canonical Modal training image is defined in the main training launcher. There are two paths, and only one of them is used in practice.
...
The inductor-cache volume is the one that actually bends the cold-start curve. Compiling a depth-52 hybrid model with attention, MoE, Mamba-3, and M2RNN blocks produces thousands of inductor entries (fx graphs, AOT-autograd decompositions, Triton kernels) and doing that from scratch on an H200 takes upwards of 25 minutes. The training launcher keeps the volume warm across runs and, when the volume is empty on a fresh deployment, seeds it in a layered fallback order:
...
The fused-kernel import probe moves from a print-side-effect into a structured field on the run receipt. The earlier ad-hoc backend-detection regex in the FA4 receipt path, which had to gain a fallback matcher for runtime observation JSON when the original pattern missed `"actual_backend": "fa4"`, is replaced by a single typed runtime-observation record emitted by the training loop itself, so receipts do not have to scrape logs to decide whether FA4 actually ran.

Volume layout is unchanged. Cache seeding is extracted into a small shared module so the nightly runner, the FA4 receipt runner, and the sparse-validation lane all use the same layered seeder, and so new launchers cannot accidentally skip it.
...
- the main Modal training launcher, nightly runner, matrix runner, and benchmark runner
- the FA4 receipt, backward-parity, and FSDP scaling runners
- the detached sparse benchmark and sparse-validation launchers
- the benchmark plan and multi-GPU status notes
