...
The measured price: the MIMO scan is register-heavy. On the reference mid-sized hybrid at MBS=8, nsys captures on an a modern accelerator host show:
...
Three observations. First, the double-backward kernel is the tall pole; it runs at ~12.5 percent occupancy at 255 regs per thread, which is the a modern accelerator compiler ceiling (`65536 / (2 * 128) = 256`). Second, forward and first-backward are both at ~6.2 percent occupancy, meaning the scan kernels are not memory-bound; they are register-bound on a compute-bound workload (arithmetic intensity on `bwd_bwd` is roughly 479 against the a modern accelerator ridge around 206). Third, the backward is larger than the forward by more than 2x, which is where optimization pressure belongs.
...
PsiV is computed from scratch in `fwd`, in `bwd_fwd`, and again inside `bwd_bwd`. Three independent materializations per forward-backward iteration of a tensor whose two inputs are stable within the iteration. The P2 design (a PSIV cache design note) is about turning that into one materialization plus two loads.

The cache is an activation checkpoint, not a hash table. Save PsiV to gmem during `fwd`, pass it into the backward kernels as an extra input, skip the recompute. Shape `(B, S, H, R, P)`, BF16, chunk-contiguous layout. For the reference shape at MBS=8 that is about 5.6 GiB of extra per-rank memory, fine inside the ~132 GiB a modern accelerator peak we run at. Expected envelope is 1.5 to 2.3 percent total TFLOP/s.
...
**Per-head information density.** MIMO at `R=4` means each head carries four channels through the same scan. Equivalent attention-based width would require either more heads (more KV cache) or a wider head dimension (more per-op compute). On a modern accelerator, the MIMO path is strictly cheaper at the same representational capacity.
...
**Backward kernels hit a real TileLang bug when TMA was enabled.** `mamba_mimo_bwd_fwd` and `mamba_mimo_bwd_bwd` used three rank-3 shared-memory descriptors, which TileLang's TMA lowering cannot handle (`InputDim() == 2` assertion, "Cannot detect TMA layout"). We fixed it by flattening 3D smem to 2D via zero-copy reshapes (`qk_dot_shared[c, r1, r2] -> [c, r1 * R + r2]`, `Q[B, S, R, G, N] -> [B, S*R, G, N]`). Correctness survived: 14 gradient tensors at `rel_err` 0.0038 to 0.0116, bit-for-bit with the TMA-off baseline inside BF16 rounding. Full perf measurement still pending an a modern accelerator slot with both forward and backward patched.
...
1. Full P1: TMA plus warp specialization on `fwd` and both backward kernels, gated by the 3D-to-2D TMA layout fix. Modeled gain is in the 5 to 10 percent range; measurement pending an a modern accelerator slot.
2. PsiV cache (P2): intra-step activation checkpoint removes two of three recomputes, modeled at 1.5 to 2.3 percent. Phase A Python prototype first, abandon if it does not move nsys.
...
4. We do not ship the P3 register split of `bwd_bwd`. The analysis that killed it is in the companion kernel-journey post; the short version is that the claimed 30 to 50 percent kernel speedup did not survive a careful line-by-line read of the reverse-scan live set.

Overall the prototype stack answered the gating question with a qualified yes: Mamba 3 MIMO is the cheaper kernel at the context lengths we train at, it represents per-head information at better density than attention does, and the costs are concentrated in `bwd_bwd` where we already have optimization paths ready. The wins are real; we just report them honestly, including the ones that came in at noise.

## What we kept and what we threw away

Kept: MIMO at `R=4` and `chunk=16` on the reference shape; TMA 3D-to-2D layout fix on the backward kernels; the PsiV cache (P2) as the next perf pass with a Phase A Python prototype gate; honest reporting of the -0.006 percent selective P1 delta; BF16 correctness gate at `rel_err` below ~0.012; attention-as-minority for sharp retrieval; dense attention at 4K-only ablations. Thrown away: the P3 register split (rejected on ROI and platform blockers); larger chunks with `R=1` (blew up `max_abs` without throughput gain); claiming 20 to 30 percent speedup from forward-only P1 (it is a wash and we said so); using the small SM_121 box as an a modern accelerator perf proxy (not viable for these shapes). The residual risk is real: the full P1 number depends on a modern accelerator time we have not yet consumed.
...
- mamba3_adoption_report_2026-03-18.md
- v4_architecture.md
- internal benchmark notes and experiment logs
